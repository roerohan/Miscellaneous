...
function isEven(num) { // Helper for conditional statement
    return num % 2 == 0;
}
...
function checkIfEven(num) {
    return new Promise( (resolve, reject) => { // This returns a new instance of promise, passed a callback function to it with resolve and reject as params
        if(isEven(num)) // if yes, resolve the promise and send back the number.
            resolve(num);
...
function checkIfEven(num) {
    return new Promise( (resolve, reject) => { // This returns a new instance of promise, passed a callback function to it with resolve and reject as params
        if(isEven(num)) // Check if number is even, if yes, resolve the promise and send back the number.
            resolve(num);
        else // If odd, reject and send back a string "Odd".
            reject("Odd");
    });
...
function returnAfterTwoSeconds (num) { // Function which returns a value after two seconds
    return new Promise( (resolve, reject) => {
        setTimeout ( () => {
            resolve(num);
        }, 2000);
...
var n = returnAfterTwoSeconds(3);
console.log(n); // Prints Promise { <pending> }, because this gets executed before the value was returned to n.

returnAfterTwoSeconds(32).then( (n) => { // Implement with promises
    console.log(n); // Prints 32 after promise is resolved
})

async function display () {
    var n = await returnAfterTwoSeconds(23); // waits till the promise is resolved and stores the resolved value in n.
